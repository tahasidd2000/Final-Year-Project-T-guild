...
contract LiquidityGeneratorToken is IERC20, Ownable {
  ...
  ITGuildswapRouter02 public TGuildswapV2Router;
  address public TGuildswapV2Pair;
  ...
  constructor(
    ...
    ITGuildswapRouter02 _TGuildswapV2Router = ITGuildswapRouter02(router_);
    // Create a uniswap pair for this new token
    TGuildswapV2Pair = ITGuildswapFactory(_TGuildswapV2Router.factory()).createPair(
      address(this),
      _TGuildswapV2Router.WETH()
    );

    // set the rest of the contract variables
    TGuildswapV2Router = _TGuildswapV2Router;
  ...
  //to recieve ETH from TGuildswapV2Router when swaping
  ...
  function _transfer(
    ...
    if (overMinTokenBalance && !inSwapAndLiquify && from != TGuildswapV2Pair && swapAndLiquifyEnabled) {
  ...
  function swapTokensForEth(uint256 tokenAmount) private {
    ...
    path[0] = address(this);
    path[1] = TGuildswapV2Router.WETH();

    _approve(address(this), address(TGuildswapV2Router), tokenAmount);

    // make the swap
    TGuildswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
      tokenAmount,
  ...
  function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
    ...
    // add the liquidity
    TGuildswapV2Router.addLiquidityETH{value: ethAmount}(
      address(this),
      tokenAmount,
