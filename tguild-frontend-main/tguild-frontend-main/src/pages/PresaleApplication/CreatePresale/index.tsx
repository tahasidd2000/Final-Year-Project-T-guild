...
const CreatePresale = ({ setHomeButtonIndex }: Props) => {
  ...
  useEffect(() => {
    async function checkIfPresaleExists() {
      const addresses: string[] = await factoryContract?.getTokenPresales(
  ...
  const paymentToken = useToken(
    currency !== "BNB"
      ? TOKEN_CHOICES[currency as keyof typeof TOKEN_CHOICES]
      : undefined
  );
  ...
  const formikPresale: FormikProps<PresaleDetails> = useFormik({
    initialValues: {
      ...
      [FieldNames.listingToken]: TOKEN_CHOICES.BNB,
    } as PresaleDetails,
      ...
      try {
        const receipt = await factoryContract.createPresale(
          ...
          ],
          {
            ...
            minBuy: parseUnits(
              (values.minBuy || 0).toString(),
              paymentToken?.decimals
            ),
            maxBuy: parseUnits(
              (values.maxBuy || 0).toString(),
              paymentToken?.decimals
            ),
            softCap: parseUnits(
              (values.softcap || 0).toString(),
              paymentToken?.decimals
            ),
            hardCap: parseUnits(
              (values.hardcap || 0).toString(),
              paymentToken?.decimals
            ),
            ...
            startPresaleTime:
              getUtcDate(
                values.startPresaleDate || "",
                values.startPresaleTime || ""
              ).getTime() / 1000,
            endPresaleTime:
              getUtcDate(
                values.endPresaleDate || "",
                values.endPresaleTime || ""
              ).getTime() / 1000,
            totalBought: "0",
            refundType: values.refundType,
            listingChoice: BigNumber.from(values.listingChoice),
        ...
        const tokenPresales: string[] = await factoryContract.getTokenPresales(
          selectedToken.address
        );
        setIsLoading(false);
        window.location.href = `/dashboard?address=${
          tokenPresales[tokenPresales.length - 1]
        }`;
  ...
  useEffect(() => {
    if (
      ...
      const presaleTokenAmount =
        formikPresale.values.presaleRate * formikPresale.values.hardcap;
      const tokensForLiquidity =
        (formikPresale.values.liquidity / 100) *
        formikPresale.values.hardcap *
        formikPresale.values.listingRate;
      formikPresale.values.tokenAmount =
        presaleTokenAmount + tokensForLiquidity;
    } else {
  ...
  return (
    <ContentWrapper>
      <Typography color="textSubtle" fontSize="14px">
