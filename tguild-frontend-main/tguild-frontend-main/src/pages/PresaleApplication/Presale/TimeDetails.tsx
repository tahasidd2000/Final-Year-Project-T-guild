...
const TimeBox = styled(Box)<{ isPresalePhase: boolean }>`
  ...
  height: 36px;
  border-radius: 4px;
  background: ${({ isPresalePhase }) =>
    isPresalePhase ? lightColors.failure : lightColors.primary};
`;
  ...
  return (
    <Card>
      ...
      {presaleInfo &&
        (presalePhase === PresalePhases.PresalePhase ||
          ...
          <>
            ...
            >
              <TimeBox
                isPresalePhase={presalePhase === PresalePhases.PresalePhase}
              >
                <StyledText color={lightColors.sidebarColor} fontWeight={700}>
                  {days}D
                </StyledText>
              </TimeBox>
              <StyledText color={lightColors.sidebarColor} fontWeight={700}>
      ...
      <Box marginTop="16px">
        ...
        <StyledText fontWeight={700} color={lightColors.sidebarColor}>
          {new Date(
            presaleInfo?.startPresaleTime.mul(1000).toNumber() || 0
          ).toUTCString()}
        </StyledText>
      </Box>
      <Box marginTop="16px">
        ...
        <StyledText fontWeight={700} color={lightColors.sidebarColor}>
          {new Date(
            presaleInfo?.endPresaleTime.mul(1000).toNumber() || 0
          ).toUTCString()}
        </StyledText>
      </Box>
    </Card>
