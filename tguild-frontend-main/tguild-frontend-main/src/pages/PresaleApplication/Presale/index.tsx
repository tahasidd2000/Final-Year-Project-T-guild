...
import { Twitter, Telegram, Email, Language } from "@mui/icons-material";
...
const Presale = ({ presaleAddress }: Props) => {
  ...
  const [openPresaleCancelModal, setOpenPresaleCancelModal] = useState(false);
  const [openPresaleFinaliseModal, setOpenPresaleFinaliseModal] =
    useState(false);
    ...
    if (presaleInfo && account && !canPresaleBeFinalized) {
      if (
        ...
        (presaleInfo.hardcap.eq(presaleInfo.totalBought) ||
          (presaleInfo.totalBought.gte(presaleInfo.softcap) &&
            presaleInfo.endPresaleTime.mul(1000).lt(Date.now())))
      ) {
  ...
  const presaleCancelHandler = async () => {
    ...
    try {
      ...
      setPresaleInfo((prevState) =>
        prevState
          ? { ...prevState, isPresaleCancelled: true, isClaimPhase: false }
          : prevState
      );
      setIsLoadingButton({
        isClicked: false,
  ...
  const withdrawCancelledTokenHandler = async () => {
    ...
    try {
      ...
      setPresaleInfo((prevState) =>
        prevState
          ? { ...prevState, isWithdrawCancelledTokens: true }
          : prevState
      );
      setIsLoadingButton({
        isClicked: false,
  ...
  const openCancelModal = () => setOpenPresaleCancelModal(true);
  const closeCancelModal = () => setOpenPresaleCancelModal(false);
  const closeCancelModalHandler = () => closeCancelModal();

  const openFinalizeModal = () => setOpenPresaleFinaliseModal(true);
  const closeFinsaliseModal = () => setOpenPresaleFinaliseModal(false);
  const closeFinsaliseModalHandler = () => closeFinsaliseModal();

  return (
    <Flex flexWrap="wrap" justifyContent="space-between">
      {presaleInfo && presaleFeeInfo && presaleToken && (
        <FinalizePresaleModal
          open={openPresaleFinaliseModal}
      ...
      <PresaleCancelModal
        open={openPresaleCancelModal}
        presaleCancelHandler={presaleCancelHandler}
        onDismiss={closeCancelModalHandler}
      />
      <ResponsiveBox1>
        <GridContainer>
          ...
          <InfoWrapper>
            ...
            <StyledText lineHeight={1} fontWeight="bold" fontSize="24px">
            ...
            {accountIsOwner &&
              !(
                presaleInfo?.isClaimPhase || presaleInfo?.isPresaleCancelled
              ) && (
                <Button
                  disabled={
                    !canPresaleBeFinalized ||
                  ...
                  loading={
                    isLoadingButton.isClicked &&
                    isLoadingButton.type === LoadingButtonTypes.Finalize
                  }
            ...
            <RowFixed marginTop="16px" marginBottom="32px">
              {projectDetails?.websiteUrl && (
                <Link
                  href={projectDetails.telegramId}
        ...
        {(isViewWhitelist || isViewContributors) && (
          <Divider marginY="24px" infoDivider />
        )}
        {isViewWhitelist && (
          <WhitelistSection
      ...
      <ResponsiveBox2>
        ...
        {contributors.length > 0 && (
          <Button
            ...
            fullWidth
            onClick={() => setIsViewContributors((prev) => !prev)}
          >
            {isViewContributors ? "Close" : "View"} Contributors List{" "}
            {`(${contributors.length})`}
          </Button>
        )}
        {whitelistAddresses.length >= 0 && (
          ...
          >
            {isViewWhitelist ? "Close" : "View"} Whitelist{" "}
          ...
          (!presaleInfo?.isPresaleCancelled ? (
            <>
              <Button
                ...
                loading={
                  isLoadingButton.isClicked &&
                  isLoadingButton.type === LoadingButtonTypes.CancelPool
                }
                fullWidth
              ...
              {isLoadingButton.type === LoadingButtonTypes.CancelPool &&
                isLoadingButton.error && (
                  <StyledText color="error" marginTop="4px" fontSize="10px">
                    {isLoadingButton.error}
                  </StyledText>
                )}
            </>
              ...
              <Button
                ...
                loading={
                  ...
                  isLoadingButton.type ===
                    LoadingButtonTypes.WithdrawCancelledTokens
                }
                disabled={
                  isLoadingButton.isClicked ||
                  isLoading ||
                  presaleInfo?.isWithdrawCancelledTokens
                }
                fullWidth
              >
