...
const ClaimTokens = ({
  ...
  const [availableTokensToClaim, setAvailableToClaim] = useState(
    BigNumber.from("0")
  );
  ...
  useEffect(() => {
    async function fetchTotalTokens() {
      setTotalTokens(
        await presaleContract?.calculateBnbToPresaleToken(
          boughtAmount,
          presaleInfo?.presaleRate
        )
      );
    }
    if (presaleContract && presaleInfo && account && boughtAmount.gt("0"))
      fetchTotalTokens();
    ...
    <Box>
      ...
      <Card>
        ...
        <Flex justifyContent="space-between">
          ...
          <StyledText color="black" fontSize="14px">{`${formatUnits(
            boughtAmount,
        ...
        <Flex justifyContent="space-between" marginBottom="8px">
          ...
          <StyledText color="black" fontSize="14px">
            {`${formatUnits(totalTokens, presaleToken?.decimals)} ${
              presaleToken?.symbol || ""
            }`}
          </StyledText>
        </Flex>
        ...
        <Flex justifyContent="center">
          {isLoadingButton.error ? (
            <StyledText color="error" fontSize="12px">
              {isLoadingButton.error}
            </StyledText>
          ) : (
            <>
              {presaleInfo?.isClaimPhase ? (
                <StyledText color="primary" fontSize="12px">
                  {totalTokens.gt("0") && totalTokens.eq(claimedTokens)
                    ? "You have claimed your tokens"
                    : "  The creator has finalized the project."}
                </StyledText>
              ) : (
                <StyledText color="black" fontSize="12px">
